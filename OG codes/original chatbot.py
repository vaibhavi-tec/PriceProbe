...
def app(user_input=None):
    ...
    while True:
        ...
        if any(word in user_input.lower() for word in ["help", "support", "guide"]):
            st.write("Chatbot: I can guide you through different websites and find great offers for the products you want to buy.")
        elif any(word in user_input.lower() for word in ["what you can do", "what help you provide", "what support is offered"]):
        ...
        if any(word in user_input.lower() for word in ["find best deal", "product comparison"]):
            ...
            if product_name:
                ...
                st.write("Amazon:", f'https://www.amazon.in/{product_name.replace(" ", "-")}/s?k={product_name.replace(" ", "+")}')
                st.write("Croma:", f'https://www.croma.com/search/?text={product_name.replace(" ", "-")}')
                st.write("")

                more_queries = st.text_input("Chatbot: Would you like to compare prices for another product? (yes/no)\n").lower()
